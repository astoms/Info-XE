...
begin
  try
    ...
    MS_Query.SQL.Clear;
    MS_Query.SQL.Add('select * from t_zd where `status`=''заказано поставщику'' ');
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        //ShowMessage('Отобраны артикула: '+article);
        try
          ...
          begin
            ...
            //ShowMessage('Из них поставки есть по: '+article2);
            if article2='' then
            begin
              try
                MS_Query2.Active:=false;
            ...
            try
              ...
              on E:Exception do ShowMessage('Ошибка #5: '+E.Message+'.');
            end;
            sum:=num_w+num_r;
            ...
            begin
              try
                ...
                on E:Exception do ShowMessage('Ошибка #6: '+E.Message+'.');
              end;
            end
            else
            begin
              try
                ...
                on E:Exception do ShowMessage('Ошибка #7: '+E.Message+'.');
              end;
            end;
          end;
        except
...
begin
  try
    MS_Query.Active:=false;
    MS_Query.SQL.Clear();
    MS_Query.SQL.Add('select `id`, `mag` from t_zd where `status` = ''заказано поставщику'' and `ext` <> ''th'' order by `date`');
    ...
    begin
      try
        ...
        while not MS_Query2.Eof do
        begin
          if MS_Query.FieldByName('mag').ASString='магазин "Mile", г.Минск, Долгиновский тракт,188 ООО "АстомСтрой"' then  //Mile
          begin
            try
              OraQuery.Active:=false;
              OraQuery.SQL.Clear;
          ...
          if MS_Query.FieldByName('mag').ASString='магазин  "Mile", г.Брест, ул. Карьерная, 9Б ООО "АстомСтрой"' then //Brest
          begin
            try
          ...
          if MS_Query.FieldByName('mag').ASString='магазин "Новоселкин", Минский р-н, пос. Боровая,1 ООО "Астомстрой"' then //Novoselk
          begin
            try
          ...
          if MS_Query.FieldByName('mag').ASString='магазин "Mile", г.Минск, пр-т Партизанский, 150А ООО "АстомСтрой"' then //Momo
          begin
            try
          ...
          if MS_Query.FieldByName('mag').ASString='Магазин "Строительный гипермаркет "Mile", г.Минск, Тимирязева, 127' then //Jdani
          begin
            try
