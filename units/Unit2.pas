...
begin
  try
    if DBGrid4.DataSource.DataSet.FieldByName('article').AsString<>'' then
    begin
      buttonSelected := MessageDlg('Выполнить очистку данных в выписке ?', mtCustom, [mbYes,mbCancel], 0);
...
begin
  try
    ...
    Auth.LoadSet.SQL.Add('insert into zv_td(`date`, `group`, `status`, `comment`) values (:TD, '''+auth.groupname+''', ''черновик'', '''')');
    ...
    ShowMessage('Ошибка: Создание заявки сейчас невозможно.'+#13+'В случае если эта ошибка происходит повторно, пожалуйста сообщите об этом системному администратору.');
    Exit;
  end;
  try
...
begin
  try
    ...
    begin
      ...
      ReadDoc(docid);
      Label17.Caption:='Документ: '+docid;
      Label18.Caption:='просмотр док-та';
...
begin
  try
    ...
    begin
      {по дате}
      if Zv.Text='все документы' then
      begin
        {статус: все документы}
        if ((Otd.Text='все отделы') or (Otd.Text='Все отделы')) then
        begin
          {отдел: все отделы}
...
begin
  if BitBtn18.Caption='Редактировать' then
  begin
    ...
    begin
      ...
      if ((Auth.SGO_Query.FieldByName('status').AsString='черновик') and (Auth.tz_moder=false)) then
      begin
      ...
      begin
        if ((Auth.SGO_Query.FieldByName('status').AsString='отправлено в товарный отдел') and (Auth.tz_moder=true)) then
        begin
          //DBGrid7.Enabled:=true;
          //BitBtn7.Enabled:=false;
          try
        ...
        begin
          ShowMessage('Редактирование документа запрещено!'+#13+'Подробнее: Вы не можете править докумен этого типа.');
        end;
      end;
    end
    else ShowMessage('Редактирование документа запрещено!'+#13+'Подробнее: Вы не являетесь владельцом документа.');
  end
  else
  begin
    //DBGrid7.Enabled:=false;
    ...
    begin
      TovSave.BitBtn2.Caption:='Сохранить (статус: заказано поставщику)';
    end
    else
    begin
      TovSave.BitBtn2.Caption:='Сохранить (статус:отправлено в товарный отдел)';
    end;
    TovSave.ShowModal;
    Label18.Caption:='просмотр док-та';
    BitBtn18.Caption:='Редактировать';
...
begin
  ...
  Excel.Visible:=false;
  Excel.WorkBooks.Add;
  Excel.WorkBooks[1].WorkSheets[1].Name := 'Отчет> Товарные ямы';
...
begin
  ...
  begin
    ...
    if finder=false then ShowMessage('Группа не найдена.');
  end;
end;
...
begin
  if Label18.Caption='черновик' then
  begin
    buttonSelected := MessageDlg('Выполнить очистку данных в выписке ?', mtCustom, [mbYes,mbCancel], 0);
    if buttonSelected = mrYes then
    begin
      ...
      DBGrid7.Cells[0,0]:='Артикул';
      DBGrid7.Cells[1,0]:='Штрих-код';
      DBGrid7.Cells[2,0]:='Наименование';
      DBGrid7.Cells[3,0]:='Поставщик';
      DBGrid7.Cells[4,0]:='Остаток';
      DBGrid7.Cells[5,0]:='Ср. сут. реал.';
      DBGrid7.Cells[6,0]:='Запас дней';
      DBGrid7.Cells[7,0]:='Дата поставки';
      DBGrid7.Cells[8,0]:='Комментарий руководителя';
      DBGrid7.Cells[9,0]:='Примечание';
    end;
  end;
end;
...
begin
  buttonSelected := MessageDlg('Удалить из документа ['+DBGrid8.DataSource.DataSet.FieldByName('name').AsString+'] ',mtConfirmation, mbOKCancel, 0);
...
var
  buttonSelected : Integer;
begin
  buttonSelected := MessageDlg('Очистить докумет №'+doc_num+' ?',mtConfirmation, mbOKCancel, 0);
  if buttonSelected = mrOK     then
  begin
    ZV_TD_Query.Active:=false;
    ZV_TD_Query.SQL.Clear;
...
var dir:string; article, place:string; List, MyPlace:TStringList; i,j:integer; ext:boolean;
begin
  DBGrid8.DataSource.DataSet.First;
  ...
  begin
    ...
    try
      ...
      begin
            place:='';
            if (Auth.MySQLSes.Connected=false) then
            begin
              Auth.MySQLSes.Connected:=true;
            end;

            try
              Place_Query3.Active:=false;
              Place_Query3.SQL.Clear;
              Place_Query3.SQL.Add('select place from serv_sklad where article = '''+article+'''');
            ...
            try
              Place_Query3.Active:=false;
              Place_Query3.SQL.Clear;
              Place_Query3.SQL.Add('select place from place_sklad where article = '''+article+'''');
              Place_Query3.Active:=true;
              if Place_Query3.RecordCount>0 then
              begin
                place:=Place+';'+Place_Query3.FieldByName('place').AsString;
      ...
      begin
        ...
        begin
          place:=place+clPlace.Items.Strings[i]+';';
          inc(i);
        end;
      end;
    except
    ...
    try
      ...
      Place_Query2.SQL.Add('update zv_td2 set place='''+place+''' where docid='''+MainForm.doc_num+''' and article='''+article+'''');
      Place_Query2.ExecSQL;
    except
    end;
    DBGrid8.DataSource.DataSet.Next;
  end;
...
var
  buttonSelected : Integer;
begin
  buttonSelected := MessageDlg('Сформировать заявку №['+MainForm.doc_num+'] ',mtConfirmation, mbOKCancel, 0);
  if buttonSelected = mrOK     then
  begin
    ZV_TD_Query.Active:=false;
    ZV_TD_Query.SQL.Clear;
    ZV_TD_Query.SQL.Add('update zv_td set status=''сформирован'' where id='''+MainForm.doc_num+'''');
    ZV_TD_Query.ExecSQL;
    Exit;
  end;
end;
...
begin
  if Auth.sklad then
  begin
    buttonSelected := MessageDlg('Заявка №['+MainForm.doc_num+'] готова ?',mtConfirmation, mbOKCancel, 0);
    if buttonSelected = mrOK     then
    begin
      ZV_TD_Query.Active:=false;
      ZV_TD_Query.SQL.Clear;
      ZV_TD_Query.SQL.Add('update zv_td set status=''собрана'' where id='''+MainForm.doc_num+'''');
      ZV_TD_Query.ExecSQL;
      Exit;
    end;
  end;
...
begin
  ...
  begin
    ...
    ChangeZPC.Caption:='Редактировать: '+DBGrid9.DataSource.DataSet.FieldByName('zpc').AsString;
    ChangeZPC.ShowModal;
  end;
end;
...
begin
  ...
  begin
    ...
    ShowMessage('Построение отчета завершено!');
  end
  else
  begin
    OpenD.Free;
...
begin
  ...
  begin
    ...
    begin
      temp:=MessageBox(handle, PChar('Нажмите YES для добавления места хранения!'), PChar('Добавить ['+AddPlace.Edit1.Text+'] в '+DBEdit1.Text), MB_YESNO+MB_ICONQUESTION);
      case temp of
        idyes:
        begin
          ...
          try
            Place_Query3.Active:=false;
            Place_Query3.SQL.Clear;
            Place_Query3.SQL.Add('select * from place_sklad where article='''+DBGrid1.DataSource.DataSet.FieldByName('article').AsString+'''');
            Place_Query3.Active:=true;
            if Place_Query3.RecordCount>0 then
            begin
              Place_Query3.Active:=false;
              Place_Query3.SQL.Clear;
              Place_Query3.SQL.Add('update place_sklad set place='''+newstring+''' where article='''+DBGrid1.DataSource.DataSet.FieldByName('article').AsString+'''');
            ...
            begin
              Place_Query3.Active:=false;
              Place_Query3.SQL.Clear;
              Place_Query3.SQL.Add('insert into place_sklad(article, count, place) values('''+DBGrid1.DataSource.DataSet.FieldByName('article').AsString+''', ''0'', '''+newstring+''')');
              //ShowMessage(Place_Query3.SQL.CommaText);
              Place_Query3.ExecSQL;
            end;
          except
          end;
...
begin
  ...
  begin
    ...
    begin
      temp:=MessageBox(handle, PChar('Нажмите YES для удаления места хранения!'), PChar('Удалить ['+clPlace.Items.Strings[clPlace.ItemIndex]+'] из '+DBEdit1.Text), MB_YESNO+MB_ICONQUESTION);
      case temp of
        idyes:
        begin
...
begin
  ...
  begin
      ...
      try
        ...
        begin
          try
            ...
            Place_Query4.Active:=false;
            Place_Query4.SQL.Clear;
            Place_Query4.SQL.Add('update tblnaserver set clPlace = '''+clPlace+'''  where clStrihkod='''+Place_Query.FieldByName('clStrihkod').AsString+''' and clName='''+Place_Query.FieldByName('clName').AsString+''' and clOperator='''+Place_Query.FieldByName('clOperator').AsString+'''');
            Place_Query4.ExecSQL;
          except
          end;
          Place_Query.Next;
        end;
    ...
    Screen.Cursor := crDefault;
    ShowMessage('Ячейки ['+Edit1.Text+'] были удалены!');
    Edit1.Text:='';
  end;
  Screen.Cursor := crDefault;
end;
...
var List, List2:TStringList; i:integer;
    article:string; clPlace,clKolish, Place, _Count, NewPlace, NewCount:string;
begin
  List:=TStringList.Create;
  ...
  begin
    Place_Query.Active:=false;
    Place_Query.SQL.Clear;
    Place_Query.SQL.Add('select clStrihkod, clPlace, clKolish from tblnaserver where clStrihkod<>'''' and len(clStrihkod)>8');
    Place_Query.Active:=true;
    Place_Query.First;
    while not Place_Query.Eof do
    begin
      ...
      Sklad_Query.Active:=false;
      Sklad_Query.SQL.Clear;
      Sklad_Query.SQL.Add('select article from smstoreunits where barcode='''+Place_Query.FieldByName('clStrihkod').AsString+'''');
      Sklad_Query.Active:=true;
      if Sklad_Query.RecordCount>0 then
      begin
      //
        article:=Sklad_Query.FieldByName('article').AsString;
        Place_Query3.Active:=false;
        Place_Query3.SQL.Clear;
        Place_Query3.SQL.Add('select place, `count` from place_sklad_temp where article = '''+article+'''');
        Place_Query3.Active:=true;
        if Place_Query3.RecordCount>0 then
        begin
          clPlace:=Place_Query.FieldByName('clPlace').AsString;
          clKolish:=Place_Query.FieldByName('clKolish').AsString;
          Place:=Place_Query3.FieldByName('place').AsString;
          _Count:=Place_Query3.FieldByName('count').AsString;
          List.Clear;
          List2.Clear;

          List.Duplicates:=dupIgnore;
          List.Delimiter:=';';
          List.DelimitedText:=clPlace;

          List2.Duplicates:=dupIgnore;
          List2.Delimiter:=';';
          List2.DelimitedText:=Place;
          ...
          begin
            try
              NewPlace:=NewPlace+List2.Strings[i]+';';
            except
            end;
            inc(i);
          end;
          ...
          try
            NewCount:=FloatToStr(StrToFloat(_Count)+StrToFloat(clKolish));
          except
            NewCount:=clKolish;
          end;
          ...
          Place_Query2.Active:=false;
          Place_Query2.SQL.Clear;
          Place_Query2.SQL.Add('update place_sklad_temp set place='''+NewPlace+''', `count`='''+NewCount+''' where article='''+article+'''');
          Place_Query2.ExecSQL;
        ...
        begin
          Place_Query2.Active:=false;
          Place_Query2.SQL.Clear;
          Place_Query2.SQL.Add('insert into place_sklad_temp(article,place,`count`) values('''+article+''', '''+Place_Query.FieldByName('clPlace').AsString+''', '''+Place_Query.FieldByName('clKolish').AsString+''')');
          Place_Query2.ExecSQL;
        end;
      //
      end;
...
begin
  ...
  begin
    ...
    begin
      if Pos('А1', Excel.Cells[i,12].Text)>0 then
      begin
        FormatList.Add('А1');
      end;
      if Pos('А2', Excel.Cells[i,12].Text)>0 then
      begin
        FormatList.Add('А2');
      end;
      if Pos('А3', Excel.Cells[i,12].Text)>0 then
      begin
        FormatList.Add('А3');
      end;
      if Pos('А4', Excel.Cells[i,12].Text)>0 then
      begin
        FormatList.Add('А4');
      end;
      if Pos('А5', Excel.Cells[i,12].Text)>0 then
      begin
        FormatList.Add('А5');
      end;
      if Pos('А6', Excel.Cells[i,12].Text)>0 then
      begin
        FormatList.Add('А6');
      end;
      if ((Excel.Cells[i,1].Text<>'') and (Excel.Cells[i,1].Text<>'Итого:') and (Excel.Cells[i,1].Text<>'итого:')) then
      begin
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          begin
            if Pos(PrintForm.Format.Items.Strings[k], Excel.Cells[i,12].Text)>0 then
            begin
              mk:=true;
              break;
            end;
      ...
      begin
        ...
        begin
          ...
          try
            ...
            article:='article = '+#39+Excel.Cells[i,15].Text+#39;
            name:=Excel.Cells[i,23].Text;
            action_price:=DelPers(Excel.Cells[i,22].Text);
            ...
            action_def:=Excel.Cells[i,19].Text;
            try
            ...
            action_def:=Excel.Cells[i,27].Text;
            try
            ...
            try
              BarcodeSet.Active:=false;
              BarcodeSet.CommandText:='select d.article, s.barcode from supermag.smstoreunits s, supermag.smcard d where d.article = s.article and d.article='''+Excel.Cells[i,15].Text+'''';
              BarcodeSet.Active:=true;
            except
            end;
            DBGrid1.Enabled:=true;
            ...
            try
              CheckQuery.Active:=false;
              CheckQuery.SQL.Clear;
              CheckQuery.SQL.Add('select price_dr, price_dk from `info_check` where article='''+Excel.Cells[i,15].Text+'''');
...
begin
  if DBGrid5.DataSource.DataSet.FieldByName('article').AsString<>'' then
  begin
    ChForm.Caption:='Выписка: '+DBGrid5.DataSource.DataSet.FieldByName('article').AsString;
    ChForm.ShowModal;
  end;
end;
...
begin
  ...
  begin
    ...
    ZPC_TABLE.Filter:='data>='''+DateToStr(DateTimePicker3.Date)+''' and data<='''+DateToStr(DateTimePicker4.Date)+''' and state<>''отгружено''';
...
begin
  ...
  BalloonHint.Title:='Режим: Обычный';
  BalloonHint.Description:='Сканирование штрих-кода товара';
...
begin
  ...
  BalloonHint.Title:='Режим: Сканирование';
  BalloonHint.Description:='Сканирование группы товаров';
...
begin
  ...
  BalloonHint.Title:='Режим: МиляКарта';
  BalloonHint.Description:='Чтение карты клиента';
...
begin
  ...
  BalloonHint.Title:='Режим: ЗПЦ';
  BalloonHint.Description:='Поиск ЗПЦ в базе';
...
begin
  try
    if ComboBox2.Text='Все заявки' then
    begin
      if ComboBox1.Text='Все отделы' then
      begin
        ZV_Read.Active:=false;
        ZV_Read.Active:=true;
...
begin
  ...
  if recieve='esc' then
  begin
    ShowMessage('Извините, ваш запрос не был выполнен.'+#13+#13+'Вероятные причины сбоя: '+#13+'1) Сервер временно недоступен, попробуйте выполнить запрос через 2-3 минуты.'+
    #13+'2) Количество запросов превысило лимит.'+#13+'3) Данная карта не существует.'+#13+#13+'В случае повтора данной ошибки, пожалуйста сообщите об этом системному администратору.');
    Exit;
  end;
...
begin
  ...
  begin
    ...
    ZV_Query.SQL.Clear;
    ZV_Query.SQL.Add('insert into t_zd(`date`, `group`, `status`, `mag`) values(:SD, '''+Auth.groupname+''', ''черновик'', '''+Auth.magazin+''')');
...
var goods:string; i,k,e,j:integer; DataSetGood:string; temp:string;  List:TStringList;
    ext:boolean; Place, Kolish:string;
begin
    ...
    begin
      ...
      try
        ...
        PropSet.CommandText:=PropSet.CommandText+'s.propid) as propid, s.propval from supermag.smcardproperties s where s.article='''+TreeSet.FieldByName('article').AsString+''' and s.propval is not null';
        PropSet.Active:=true;
      except
      end;
      ...
      try
        try
          ...
          begin
            ...
            begin
              Sklad_Query.Active:=false;
              Sklad_Query.SQL.Clear;
              Sklad_Query.SQL.Add('select barcode from smstoreunits where article='''+DBGrid1.DataSource.DataSet.FieldByName('article').AsString+'''');
              Sklad_Query.Active:=true;
              ...
              begin
                ...
                begin
                  try
                    Place_Query.Active:=false;
                    Place_Query.SQL.Clear;
                    Place_Query.SQL.Add('select clPlace, clKolish from tblnaserver where clStrihkod='''+Sklad_Query.FieldByName('barcode').AsString+'''');
                    Place_Query.Active:=true;
                  except
                  ...
                  try
                    ...
                    begin
                      while not Place_Query.Eof do
                      begin
                        Place:=Place+Place_Query.FieldByName('clPlace').AsString+';';
                        try
                          Kolish:=FloatToStr(RoundTo((StrToFloat(Kolish)+StrToFloat(Place_Query.FieldByName('clKolish').AsString)),-3));
                        except
                          Kolish:=Place_Query.FieldByName('clKolish').AsString;
                        end;
                        Place_Query.Next;
                      end;
                    end;
        ...
        try
          ...
          begin
            ...
            try
              Place_Query3.Active:=false;
              Place_Query3.SQL.Clear;
              Place_Query3.SQL.Add('select place from serv_sklad where article = '''+DBGrid1.DataSource.DataSet.FieldByName('article').AsString+'''');
              Place_Query3.Active:=true;
              if Place_Query3.RecordCount>0 then
              begin
                Place:=Place+Place_Query3.FieldByName('place').AsString;
        ...
        //List.Duplicates:=dupIgnore;
        List.Delimiter:=';';
        List.DelimitedText:=Place;
        //ShowMessage('NowError');
        clPlace.Items.Clear;
        i:=0;
        while i<>List.Count do
        begin
          if ((List.Strings[i]<>'') and (List.Strings[i]<>'0')) then
          begin
            j:=0;
            ext:=false;
        ...
        Label24.Caption:='Количество на складе: '+Kolish;

      except
        on E:Exception do ShowMessage('Ошибка all: '+E.Message);
      end;

        //места хранения конец
  ...
  try
    ...
    begin
      ...
      begin
        ...
        try
          if (ZV_TD_Query.FieldByName('status').AsString='черновик') then
          begin
            ZvAdd.Caption:='Введите количество ['+DBGrid1.DataSource.DataSet.FieldByName('article').AsString+']:';
            ZvAdd.ShowModal;
          end
          else ShowMessage('Заявка уже сформирована или собрана.'+#13+'Добавление артикула в список невозможно!');
        except
        end;
      end;
      Exit;
    ...
    begin
      if ((Panel25.Visible=true) and (BitBtn18.Caption='Сохранить') and (Auth.tz_moder=false)) then
      begin
        ...
        begin
          try
            ...
            //if Tov_Query.RecordCount=0 then showmessage('поля нет');
          ...
          try
            ...
            begin
              ...
              Tov_Query.SQL.Add('and u.name <> ''Астомстрой ООО '' ');
    ...
    begin
      try
        if DBGrid1.DataSource.DataSet.FieldByName('article').AsString<>'' then
        begin
          EnterForm.Caption:='Выписка: '+DBGrid1.DataSource.DataSet.FieldByName('article').AsString;
          EnterForm.ShowModal;
        end;
      except
      end;
    ...
    begin
      try
        ...
        PropSet.CommandText:=PropSet.CommandText+'''User.Import_Group'', ''Импорт: Группа в старой базе'', ''User.Width'', ''Габариты: Ширина, мм'', ''User.Height'', ''Габариты: Высота, мм'', ''User.Deep'', ''Габариты: Глубина, мм'', ''55'', ''Площадь, м2'', ''56'', ''Количество в упаковке, шт'', ';
        PropSet.CommandText:=PropSet.CommandText+'''57'', ''Акционное название'', ''58'', ''Класс'', ''59'', ''Толщина защитного слоя'',''60'', ''Срок службы'', ';
        PropSet.CommandText:=PropSet.CommandText+'s.propid) as propid, s.propval from supermag.smcardproperties s where s.article='''+DBGrid1.DataSource.DataSet.FieldByName('article').AsString+''' and s.propval is not null';
        PropSet.Active:=true;
        ...
        begin
          if PropSet.FieldByName('propid').AsString='Срок службы' then
          begin
            srok:=noBadChar(PropSet.FieldByName('propval').AsString);
          end;
          if PropSet.FieldByName('propid').AsString='Состав' then
          begin
            sost:=noBadChar(PropSet.FieldByName('propval').AsString);
          end;
          if PropSet.FieldByName('propid').AsString='User.MP' then
          begin
            user_mp:=noBadChar(PropSet.FieldByName('propval').AsString);
          end;
          if ((PropSet.FieldByName('propid').AsString='Толщина покрытия') or (PropSet.FieldByName('propid').AsString='Толщина')) then
          begin
            p_width:=noBadChar(PropSet.FieldByName('propval').AsString);
          end;
          if PropSet.FieldByName('propid').AsString='Толщина защитного слоя' then
          begin
            pr_width:=noBadChar(PropSet.FieldByName('propval').AsString);
          end;
          if PropSet.FieldByName('propid').AsString='Размер' then
          begin
            size:=noBadChar(PropSet.FieldByName('propval').AsString);
          end;
          if PropSet.FieldByName('propid').AsString='Габариты: Ширина, мм' then
          begin
            size_2:=size_2+noBadChar(PropSet.FieldByName('propval').AsString)+'x';
          end;
          if PropSet.FieldByName('propid').AsString='Габариты: Высота, мм' then
          begin
            size_2:=size_2+noBadChar(PropSet.FieldByName('propval').AsString)+'x';
          end;
          if PropSet.FieldByName('propid').AsString='Габариты: Глубина, мм' then
          begin
            size_2:=size_2+noBadChar(PropSet.FieldByName('propval').AsString)+'x';
          end;
          if PropSet.FieldByName('propid').AsString='Цвета' then
          begin
            color:=noBadChar(PropSet.FieldByName('propval').AsString);
          end;
          if PropSet.FieldByName('propid').AsString='Площадь, м2' then
          begin
            plosh:=noBadChar(PropSet.FieldByName('propval').AsString);
          end;
          if PropSet.FieldByName('propid').AsString='Количество в упаковке, шт' then
          begin
            count_upak:=noBadChar(PropSet.FieldByName('propval').AsString);
          end;
          if PropSet.FieldByName('propid').AsString='Класс' then
          begin
            _class:=noBadChar(PropSet.FieldByName('propval').AsString);
          end;
          if PropSet.FieldByName('propid').AsString='Торговая марка' then
          begin
            trademark:=noBadChar(PropSet.FieldByName('propval').AsString);
          end;
          if PropSet.FieldByName('propid').AsString='Дополнительно' then
          begin
      ...
      try
        ...
        begin
          ...
          try
            ...
            //On E:Exception do ShowMessage('Ошибка добавления артикула: '+E.Message+'.');
          end;
          try
            CheckTable.Active:=false;
            CheckTable.Active:=true;
...
begin
  ...
  begin
    ...
    if H > DBGrid7.RowHeights[ARow] then DBGrid7.RowHeights[ARow] := H; //увеличиваем
  end;
...
begin
  ...
  if Label18.Caption='отправлено в товарный отдел' then
  begin
  ...
  if ((Label18.Caption='заказано поставщику') or (Label18.Caption='просмотр док-та')) then
  begin
    key:=#0;
  end;
end;
...
var i:integer;
begin
  if BitBtn18.Caption='Сохранить' then
  begin
    if Auth.tz_moder=true then
    begin
...
begin
  ...
  begin
    ZvChange.Caption:='Введите количество ['+DBGrid8.DataSource.DataSet.FieldByName('article').AsString+']:';
...
var dir, article, place:string; i:integer;
begin
  if PageControl1.ActivePageIndex=6 then
  begin
    dir:=ExtractFilePath(ParamStr(0));
  ...
  begin
    try
      ...
      begin
        ...
        begin
          TreeSet.Filter:='tree like '''+group+'%'' and accepted=''Активна''';
        end
        else
        begin
...
begin
  ...
  begin
    ...
    Grid.Cells[0,0]:='Группа';
    Grid.Cells[1,0]:='Артикул';
    Grid.Cells[2,0]:='Штрих-код';
    Grid.Cells[3,0]:='Наименование';
    Grid.Cells[4,0]:='Поставщик';
    Grid.Cells[5,0]:='Остаток';
    Grid.Cells[6,0]:='Ср. сут. реал.';
    Grid.Cells[7,0]:='Запас дней';
    Grid.Cells[8,0]:='Дата поставки';
    Grid.Cells[9,0]:='Комментарий руководителя';
    Grid.Cells[10,0]:='Примечание';
    Grid.Cells[11,0]:='Ext';
    ...
    DBGrid7.Cells[0,0]:='Группа';
    DBGrid7.Cells[1,0]:='Артикул';
    DBGrid7.Cells[2,0]:='Штрих-код';
    DBGrid7.Cells[3,0]:='Наименование';
    DBGrid7.Cells[4,0]:='Поставщик';
    DBGrid7.Cells[5,0]:='Остаток';
    DBGrid7.Cells[6,0]:='Ср. сут. реал.';
    DBGrid7.Cells[7,0]:='Запас дней';
    DBGrid7.Cells[8,0]:='Дата поставки';
    DBGrid7.Cells[9,0]:='Комментарий руководителя';
    DBGrid7.Cells[10,0]:='Примечание';
...
begin
  Place_Query3.Active:=false;
  Place_Query3.SQL.Clear;
  Place_Query3.SQL.Add('select * from `zv_td` where `status`=''собрана'' and `statmes`=''false'' and `group`='''+Auth.groupname+'''');
  Place_Query3.Active:=true;
  if Place_Query3.RecordCount<>0 then
  begin
    i:=0;
    ...
    begin
      ...
      AForm[i].Caption:='Склад:';
      AForm[i].num:=Place_Query3.FieldByName('id').AsString;
      AForm[i].name:=Place_Query3.FieldByName('status').AsString;
      AForm[i].Label1.Caption:='Ваша заявка №'+Place_Query3.FieldByName('id').AsString+' обработана.';
      AForm[i].Show;
      Place_Query3.Next;
      inc(i);
    end;
...
begin
  ...
  frmPreview.Caption:='Предварительный просмотр: ';
...
begin
  try
    Label10.Caption:='Документ '+RGrid.DataSource.DataSet.FieldByName('id').AsString+':';
    ...
    begin
      ...
      if MainForm.ZV_TD_Query.FieldByName('status').AsString='собрана' then
      begin
        MainForm.BitBtn34.Enabled:=false;
      end;
    end;
...
begin
  ...
  begin
    ...
    if ((RadioButton4.Checked=true) and (Auth.ServerDB.Text<>'MILE_MINSK')) then
    begin
      try
        if (Access.Connected = false) then Access.Connected:=true;
      except
    ...
    begin
      ...
      try
        try
          ...
          OraZPC.SQL.Add('FROM SMDateDocs a, smdocuments d, smclientinfo c, smstorelocations l, smspecor s, smcard ds, (select g.article, sum(g.quantity) as quantity, g.storeloc from smgoods g group by g.article, g.storeloc) gs, smstoreunits sb ');
...
begin
  ...
  if H > (Sender as TStringGrid).RowHeights[ARow] then
    (Sender as TStringGrid).RowHeights[ARow] := H + 3;  //увеличиваем
...
begin
  try
    ...
    begin
      TreeSet.Filtered:=false;
      TreeSet.Filter:='accepted=''Активна''';
      TreeSet.Filtered:=true;
      if FindEdit.Text<>'' then
      begin
        Search.Caption:='Найти';
...
begin
  try
    ...
    begin
      ...
      Screen.Cursor := crAppStart;
      TreeSet.Active:=false;
      Cartoons.Caption:='Карточек отобрано: Все';
      Screen.Cursor := crDefault;

      if ToggleAct.State=TToggleSwitchState.tssOff then
      begin
        TreeSet.Active:=false;
        TreeSet.CommandText:='select d.article, d.name, d.mesname, d.country, DECODE(accepted, ''1'', ''Активна'', ''2'', ''Исключена'', ''Новая'') as accepted, ';
        TreeSet.CommandText:=TreeSet.CommandText+'DECODE(datatype, ''0'', ''Товар'', ''1'', ''Услуга'', ''2'', ''Деньги'', ''3'', ''Тара'', ''4'', ''Инвентарь'', ''5'', ''Набор'', ''#Н/Д'') as datatype, s.tree, s.name as gname from supermag.smcard d, supermag.sacardclass s WHERE s.id = d.idclass and accepted=1';
        TreeSet.Active:=true;
      end
      else
      begin
        TreeSet.Active:=false;
        TreeSet.CommandText:='select d.article, d.name, d.mesname, d.country, DECODE(accepted, ''1'', ''Активна'', ''2'', ''Исключена'', ''Новая'') as accepted, ';
        TreeSet.CommandText:=TreeSet.CommandText+'DECODE(datatype, ''0'', ''Товар'', ''1'', ''Услуга'', ''2'', ''Деньги'', ''3'', ''Тара'', ''4'', ''Инвентарь'', ''5'', ''Набор'', ''#Н/Д'') as datatype, s.tree, s.name as gname from supermag.smcard d, supermag.sacardclass s WHERE s.id = d.idclass';
        TreeSet.Active:=true;
      end;
      Cartoons.Caption:='Карточек отобрано: '+IntToStr(TreeSet.RecordCount);
      }
    ...
    begin
      try
        ...
        begin
          ...
          TreeSet.Active:=false;
          TreeSet.CommandText:='select d.article, d.name, d.mesabbrev as mesname, d.country, DECODE(accepted, ''1'', ''Активна'', ''2'', ''Исключена'', ''Новая'') as accepted, ';
          TreeSet.CommandText:=TreeSet.CommandText+'DECODE(datatype, ''0'', ''Товар'', ''1'', ''Услуга'', ''2'', ''Деньги'', ''3'', ''Тара'', ''4'', ''Инвентарь'', ''5'', ''Набор'', ''#Н/Д'') as datatype, s.tree, s.name as gname from supermag.smcard d, supermag.sacardclass s WHERE s.id = d.idclass and s.tree LIKE '''+tmp+'%'' and accepted=1';
          TreeSet.Active:=true;
        ...
        begin
          ...
          TreeSet.Active:=false;
          TreeSet.CommandText:='select d.article, d.name, d.mesabbrev as mesname, d.country, DECODE(accepted, ''1'', ''Активна'', ''2'', ''Исключена'', ''Новая'') as accepted, ';
          TreeSet.CommandText:=TreeSet.CommandText+'DECODE(datatype, ''0'', ''Товар'', ''1'', ''Услуга'', ''2'', ''Деньги'', ''3'', ''Тара'', ''4'', ''Инвентарь'', ''5'', ''Набор'', ''#Н/Д'') as datatype, s.tree, s.name as gname from supermag.smcard d, supermag.sacardclass s WHERE s.id = d.idclass and s.tree LIKE '''+tmp+'%''';
          TreeSet.Active:=true;
...
begin
  ...
  try
    ...
    _TreeSet.CommandText:='select d.article, d.name, d.mesname, d.country, DECODE(accepted, ''1'', ''Активна'', ''2'', ''Исключена'', ''Новая'') as accepted, DECODE(datatype, ''0'', ''Товар'', ''1'', ''Услуга'', ''2'', ''Деньги'', ''3'', ''Тара'', ''4'', ''Инвентарь'', ''5'', ''Набор'', ''#Н/Д'') ';
    ...
    MainForm.Cartoons.Caption:='Карточек отобрано: '+IntToStr(_TreeSet.RecordCount);
  except
    MainForm.Cartoons.Caption:='Карточек отобрано: 0';
  end;
  ...
  try
    ...
    _GoodSet.CommandText:=_GoodSet.CommandText+'decode(storeloc, ''2'', ''Маг. "Новоселкин", Минский р-н, пос. Боровая,1, 223053'', ''8'', ''Маг. "Mile", г.Минск, Долгиновский тракт,188, 220053'', ''11'', ''Маг. "Mile", г.Брест, ул. Карьерная, 9Б, 224022'',';
    _GoodSet.CommandText:=_GoodSet.CommandText+' ''16'', ''Маг. "Mile", г.Могилев, Минское Шоссе,31, 212039'', ''18'', ''Маг. "Mile", г.Минск, пр-т Партизанский, 150А'',';
    _GoodSet.CommandText:=_GoodSet.CommandText+' ''10'', ''Склад "Mile" Долгиновский, 220053 г.Минск, Долгиновский тракт, 188'', ''14'',';
    _GoodSet.CommandText:=_GoodSet.CommandText+' ''Логистический центр "Mile", 220113 г.Минск, ул. Лукьяновича, 10'', ''15'', ''Астомстрой ЖД'', ''17'', ''Склад возврата и брака "Логистический центр", ул. Лукьяновича, 10'', ''-'') as storeloc';
  ...
  MainForm.TreeSet.Filter:='accepted=''Активна''';
