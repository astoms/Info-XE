...
begin
  ...
  try
    ...
    begin
      //ShowMessage('Сохраняем документ...');
      if MainForm.DBGrid7.Cells[0, MainForm.DBGrid7.RowCount-1]<>'' then
      begin
        ext:=false;
        i:=1;
        ...
        if ext then
        begin
          ShowMessage('Внимание! Сохранение документа не выполнено.'+#13+'Подробнее: '+'Пожалуйста проставьте даты поставки товара!');
          Exit;
        end;
        try
          Auth.SGO_Query.Active:=false;
  ...
  try
    if BitBtn2.Caption='Сохранить (статус:отправлено в товарный отдел)' then
    begin
      Auth.SGO_Query.Active:=false;
      Auth.SGO_Query.SQL.Clear;
      Auth.SGO_Query.SQL.Add('update t_zd set status=''отправлено в товарный отдел'' where id='''+MainForm.docid+'''');
      Auth.SGO_Query.ExecSQL;
    end
    else
    begin
      Auth.SGO_Query.Active:=false;
      Auth.SGO_Query.SQL.Clear;
      Auth.SGO_Query.SQL.Add('update t_zd set status=''заказано поставщику'' where id='''+MainForm.docid+'''');
      Auth.SGO_Query.ExecSQL;
    end;
  except
    //on e:exception do ShowMessage(e.Message);
