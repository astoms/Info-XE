...
begin
  ...
  try
    ...
    ORA_QUERY.SQL.Add('where d.id = ''ЗПЦ'+ZPCNum.Text+'''');
  ...
  ORA_Query.First;
  if ORA_Query.FieldByName('id').AsString='ЗПЦ'+ZPCNum.Text then
  begin
  ...
  try
    ...
    MySQL_Query.SQL.Add('insert into zpc_table(`zpc`, `from`, `to`, `data`, `timed`, `whois`, `count_e`, `state`) values('''+id+''', '''+froms+''', '''+tos+''', CURDATE(), CURTIME(), '''+Auth.LoginDB.Text+''', ''-'', ''в отборке'')');
    MySQL_Query.ExecSQL;
  except
    ShowMessage('Произошла ошибка: Пожалуйста сообщите об этом системному администратору!');
  end;
