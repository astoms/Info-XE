...
begin
  ...
  begin
    temp:=MessageBox(handle, PChar('Вы действительно решили добавить новый вид цены с именем "'+LabeledEdit1.Text+'" и значением "'+LabeledEdit2.Text+'" в базу данных ?'), PChar('Добавление в "Виды цен":'), MB_YESNO+MB_ICONQUESTION);
    case temp of
      idyes:
      begin
...
var
 temp:Word;
begin
  temp:=MessageBox(handle, PChar('Вы действительно решили удалить вид цены "'+DBGrid1.DataSource.DataSet.FieldByName('name').AsString+'" из базы данных ?'), PChar('Удаление значения из "Виды цен":'), MB_YESNO+MB_ICONQUESTION);
  case temp of
    idyes:
      begin
        PriceQuery.Active:=false;
