...
var
 temp:Word;
begin
  temp:=MessageBox(handle, PChar('¬ы действительно решили удалить торговую точку "'+DBGrid1.DataSource.DataSet.FieldByName('name').AsString+'" из базы данных ?'), PChar('”даление значени€ из "“оргова€ точка":'), MB_YESNO+MB_ICONQUESTION);
  case temp of
    idyes:
      begin
...
begin
  if (Edit1.Text<>'') then
  begin
    temp:=MessageBox(handle, PChar('¬ы действительно решили добавить торговую точку с именем "'+Edit1.Text+'" в базу данных ?'), PChar('ƒобавление торговой точки:'), MB_YESNO+MB_ICONQUESTION);
    case temp of
      idyes:
      begin
        TradeQuery.Active:=false;
